...
| Required CodeQL-compatible check | **Pending execution** | MIT removes the previous standalone-CLI license restriction for this project, but no CodeQL result exists yet. Ledger EV-016. |
...
1. **CodeQL execution.** Run the official CodeQL CLI against the exact MIT-licensed candidate and retain a reviewed result. Semgrep and local tests do not replace CodeQL.
