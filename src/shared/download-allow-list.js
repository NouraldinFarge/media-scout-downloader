...
/**
 * Positive download policy.
 *
 * The important distinction is action scope:
 * - direct top-level file save: hand the exact URL to chrome.downloads.
 * - manifest-only save: hand the exact manifest URL to chrome.downloads.
 * - merge/convert: fetch and reuse playlist component URLs in a page context.
 *
 * Signed/tokenized URLs are allowed only for the first two scopes. They remain
 * blocked for stream components, raw segments, HLS merge/remux, helper notes,
 * and anything that would reuse a protected component URL.
 */
export const DOWNLOAD_ALLOW_RULES = Object.freeze([
  {
    id: 'progressive-final-video',
    mediaTypes: [MEDIA_TYPES.VIDEO],
    strategies: [STRATEGY_NAMES.DIRECT_FILE, STRATEGY_NAMES.HTML_MEDIA_SOURCE],
    allowedUrlKinds: ['plain-http', 'signed-top-level-http'],
    allowWhen: 'HTTP(S) URL, enabled video extension or video MIME type, no DRM/encryption/auth/paywall marker. Signed top-level URLs may be passed unchanged to Chrome Downloads.',
    blockedWhen: 'Encrypted/DRM/auth/paywall/CORS/access-control marker, unsafe scheme, disabled type, or unknown non-media type.',
    output: 'Original video file.'
  },
  {
    id: 'progressive-final-audio',
    mediaTypes: [MEDIA_TYPES.AUDIO],
    strategies: [STRATEGY_NAMES.DIRECT_FILE, STRATEGY_NAMES.HTML_MEDIA_SOURCE],
    allowedUrlKinds: ['plain-http', 'signed-top-level-http'],
    allowWhen: 'HTTP(S) URL, enabled audio extension or audio MIME type, no DRM/encryption/auth/paywall marker. Signed top-level URLs may be passed unchanged to Chrome Downloads.',
    blockedWhen: 'Encrypted/DRM/auth/paywall/CORS/access-control marker, unsafe scheme, disabled type, or unknown non-media type.',
    output: 'Original audio file.'
  },
  {
    id: 'page-local-blob',
    mediaTypes: [MEDIA_TYPES.VIDEO, MEDIA_TYPES.AUDIO],
    strategies: [STRATEGY_NAMES.BLOB_PAGE_DOWNLOAD],
    allowedUrlKinds: ['same-page-blob'],
    allowWhen: 'blob: URL created by the page that is not marked as an MSE/player buffer placeholder and can be saved from the same tab/frame.',
    blockedWhen: 'MSE/player buffer placeholder, stale tab/frame, or page-context save failure.',
    output: 'Page-local blob file.'
  },
  {
    id: 'safe-hls-mpegts-merge',
    mediaTypes: [MEDIA_TYPES.HLS],
    strategies: [STRATEGY_NAMES.HLS_SEGMENT_MERGE],
    allowedUrlKinds: ['plain-http'],
    allowWhen: 'HTTP(S) .m3u8 playlist with no signed/tokenized top-level URL, no encryption keys, no protected component URLs, no fMP4 #EXT-X-MAP, no #EXT-X-BYTERANGE layout, and normal page-context fetch access succeeds.',
    blockedWhen: 'Encrypted HLS, signed/tokenized playlist/variant/segment/audio URLs, fMP4/CMAF, byte-range media, auth/paywall/CORS/access-control failure, or missing live page context.',
    output: 'MP4 remux when compatible, otherwise timestamp-aware or raw MPEG-TS according to the chosen HLS method.'
  },
  {
    id: 'hls-playlist-file',
    mediaTypes: [MEDIA_TYPES.HLS],
    strategies: [STRATEGY_NAMES.HLS_PLAYLIST],
    allowedUrlKinds: ['plain-http', 'signed-top-level-http'],
    allowWhen: 'Top-level HTTP(S) playlist URL can be saved as text. Signed top-level playlist URLs are allowed only for playlist-file saving, not segment fetching or merging.',
    blockedWhen: 'Invalid URL, unsupported scheme, or Chrome download failure.',
    output: 'M3U8 playlist text.'
  },
  {
    id: 'hls-external-helper-notes',
    mediaTypes: [MEDIA_TYPES.HLS],
    strategies: [STRATEGY_NAMES.HLS_EXTERNAL_HELPER],
    allowedUrlKinds: ['plain-http'],
    allowWhen: 'Non-protected HTTP(S) HLS playlist URL can be written into an explicit local helper-notes file after user confirmation. This does not fetch, decrypt, or merge segments.',
    blockedWhen: 'Encrypted, signed/tokenized, protected, auth, paywall, CORS/access-control, unsupported scheme, or unconfirmed raw URL exposure.',
    output: 'TXT helper notes with the playlist URL and a local-tool command example.'
  },
  {
    id: 'dash-manifest-file',
    mediaTypes: [MEDIA_TYPES.DASH],
    strategies: [STRATEGY_NAMES.DASH_MANIFEST],
    allowedUrlKinds: ['plain-http', 'signed-top-level-http'],
    allowWhen: 'Top-level HTTP(S) MPD URL can be saved as XML, including when ContentProtection is present. Segment fetching, decryption, and merging are never attempted.',
    blockedWhen: 'Invalid URL, unsupported scheme, or Chrome download failure.',
    output: 'MPD manifest XML.'
  },
  {
    id: 'other-stream-manifest-file',
    mediaTypes: [MEDIA_TYPES.STREAM, MEDIA_TYPES.PLAYLIST],
    strategies: [STRATEGY_NAMES.DIRECT_FILE],
    allowedUrlKinds: ['plain-http', 'signed-top-level-http'],
    allowWhen: 'HTTP(S) manifest/playlist URL, enabled extension/MIME, and no hard DRM/encryption/auth/paywall marker. Saved as manifest text only.',
    blockedWhen: 'Unsafe scheme, disabled type, unknown non-manifest type, or hard protection marker.',
    output: 'Manifest or playlist file.'
  },
  {
    id: 'standalone-transport-stream-file',
    mediaTypes: [MEDIA_TYPES.SEGMENT],
    strategies: [STRATEGY_NAMES.DIRECT_FILE],
    allowedUrlKinds: ['plain-http', 'signed-top-level-http-with-strong-final-file-hint'],
    allowWhen: 'MPEG-TS URL that looks like a standalone file, such as a <video src> .ts file or a large final transport-stream response, with no playlist/protection association. Signed top-level URLs are allowed only when the item has strong final-file hints.',
    blockedWhen: 'Playlist-associated stream component, encrypted/protected association, low-latency partial segment, fMP4 segment, unsafe scheme, disabled type, or signed URL without a strong standalone-file hint.',
    output: 'Original MPEG-TS file.'
  },
  {
    id: 'standalone-segment-fragment',
    mediaTypes: [MEDIA_TYPES.SEGMENT],
    strategies: [STRATEGY_NAMES.DIRECT_FILE],
    allowedUrlKinds: ['plain-http'],
    allowWhen: 'HTTP(S) segment URL, enabled extension/MIME, no encrypted-playlist association, and no signed/tokenized URL hints.',
    blockedWhen: 'Signed/tokenized segment URLs, encrypted/protected stream association, unsafe scheme, disabled type, low-latency partial segment, or unknown segment type.',
    output: 'Raw segment file; not assembled video.'
  },
  {
    id: 'metadata-file',
    mediaTypes: [MEDIA_TYPES.METADATA],
    strategies: [STRATEGY_NAMES.DIRECT_FILE],
    allowedUrlKinds: ['plain-http', 'signed-top-level-http'],
    allowWhen: 'HTTP(S) URL, user-enabled metadata extension or MIME type, and no hard auth/paywall/access-control marker. JSON/XML metadata is disabled by default to avoid noise, but it can be downloaded when explicitly enabled.',
    blockedWhen: 'Unsafe scheme, disabled metadata type, unknown metadata type, or hard auth/paywall/access-control marker.',
    output: 'Original metadata file.'
  },
  {
    id: 'companion-file',
    mediaTypes: [MEDIA_TYPES.SUBTITLE, MEDIA_TYPES.IMAGE],
    strategies: [STRATEGY_NAMES.DIRECT_FILE],
    allowedUrlKinds: ['plain-http', 'signed-top-level-http'],
    allowWhen: 'HTTP(S) URL, enabled image/subtitle extension or MIME type. Signed top-level URLs may be passed unchanged to Chrome Downloads.',
    blockedWhen: 'Unsafe scheme, disabled type, unknown companion type, or hard auth/paywall/access-control marker.',
    output: 'Original companion file.'
  }
]);
